...
void Game::select_opponent ()
{
  ...
  cout << OP_MSG << endl;
  cin >> input;
  while (input != "CPU" && input != "cpu"
  && input != "P2P" && input != "p2p")
  {
	cout << OP_ERR_MSG << endl;
	cin >> input;
  }
  if (input == "CPU" || input == "cpu")
  { _game_choice = "cpu"; run_cpu_game(); }

  if (input == "P2P" || input == "p2p")
  { _game_choice = "p2p"; run_p2p_game(); }
}
...
int Game::minimax (Board& board, int depth, bool is_maxi)
{
  ...
  if (tmp_board.board_is_full() || win (board)) { return evaluate_board(board); }
  if (is_maxi)
  {
